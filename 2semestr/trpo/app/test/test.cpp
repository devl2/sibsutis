...
void fonwin(Vector2f& pos)
{
    ...
    if (backsprite.getGlobalBounds().contains(pos.x, pos.y)) {
        backsprite.setColor(Color(255, 250, 0));
    } else {
...
{
    if (p0S.getGlobalBounds().contains(pos.x, pos.y)) {
        poswin = 0;
    }
    if (p1S.getGlobalBounds().contains(pos.x, pos.y)) {
        poswin = 1;
    }
    if (p2S.getGlobalBounds().contains(pos.x, pos.y)) {
        poswin = 2;
    }
    if (p3S.getGlobalBounds().contains(pos.x, pos.y)) {
        poswin = 3;
    }
}
...
void mode(Vector2f& pos)
{
    ...
    if (playersprite.getGlobalBounds().contains(pos.x, pos.y)) {
        playersprite.setColor(Color(255, 250, 0));
    } else {
        playersprite.setColor(Color::White);
    }
    if (clubsprite.getGlobalBounds().contains(pos.x, pos.y)) {
        clubsprite.setColor(Color(255, 250, 0));
    } else {
        clubsprite.setColor(Color::White);
    }
    if (countrysprite.getGlobalBounds().contains(pos.x, pos.y)) {
        countrysprite.setColor(Color(255, 250, 0));
    } else {
        countrysprite.setColor(Color::White);
    }
...
bool mockLoadFromFile(sf::Texture& texture, const std::string& filename)
{
    return texture.create(
            100, 100); // Создание фиктивной текстуры размером 100x100
}
