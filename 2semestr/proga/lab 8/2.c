...
void fakult_stat(const room rooms[], int rooms_num, const char fakult[][6], int num_fakult)
{
    for(int i = 0; i < num_fakult; i++)
    {
        ...
        printf("факультет - %s, кол-во комнат - %d, кол-во студентов - %d, средняя площадь - %.2f\n", fakult[i], total_rooms, total_stud, sred_ploshad);
    }
}
...
int main()
{
    ...
    fakult_count(rooms, fakult, rooms_num, &num_fakult);
    ...
    fakult_stat(rooms, rooms_num, fakult, num_fakult);
